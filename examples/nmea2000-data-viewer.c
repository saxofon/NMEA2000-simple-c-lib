	...
	switch (pgn) {
		...
		case PGN_SEATALK_PILOT_HEADING_ID:
			if (msg->data.seatalk_pilot_heading.heading_true != 0xFFFF)
				nmea2000_data.autopilot_heading_true     = (float)(msg->data.seatalk_pilot_heading.heading_true)*0.0001f;
			if (msg->data.seatalk_pilot_heading.heading_magnetic != 0xFFFF)
				nmea2000_data.autopilot_heading_magnetic = (float)(msg->data.seatalk_pilot_heading.heading_magnetic)*0.0001f;
			break;
		case PGN_SYSTEM_TIME_ID:
			nmea2000_time2tm(msg->data.system_time.date, msg->data.system_time.time, &nmea2000_data.tm);
			break;
		case PGN_RUDDER_ID:
			nmea2000_data.rudder_angle = (float)(msg->data.rudder.position)*0.0001f;
			break;
		case PGN_VESSEL_HEADING_ID:
			nmea2000_data.heading = (float)(msg->data.vessel_heading.heading)*0.0001f;
			break;
		case PGN_MAGNETIC_VARIATION_ID:
			...
			MSG_DUMP("  variation                 : %f\n", (float)(msg->data.magnetic_variation.variation)*0.0001f);
			break;
		case PGN_SPEED_ID:
			nmea2000_data.STW = (float)(msg->data.speed.speed_ref_water)*0.01f;
			//nmea2000_data.SOG = msg->data.speed.speed_ref_ground)*0.01f;
			break;
		case PGN_WATER_DEPTH_ID:
			nmea2000_data.depth = (float)(msg->data.water_depth.depth)*0.01f;
			// %0.4f m,  msg->data.water_depth.offset*0.001
			break;
		case PGN_POSITION_RAPID_UPDATE_ID:
			nmea2000_data.longitude = (float)(msg->data.position_rapid_update.longitude)*PGN_POSITION_SCALE;
			nmea2000_data.latitude = (float)(msg->data.position_rapid_update.latitude)*PGN_POSITION_SCALE;
			break;
		case PGN_COGSOG_RAPID_UPDATE_ID:
			if (msg->data.cogsog_rapid_update.cog != 0xFFFF)
				nmea2000_data.COG = (float)(msg->data.cogsog_rapid_update.cog)*0.0001f;
			if (msg->data.cogsog_rapid_update.sog != 0xFFFF)
				nmea2000_data.SOG = (float)(msg->data.cogsog_rapid_update.sog)*0.01f;
			break;
		...
		case PGN_CROSSTRACK_ERROR_ID:
			...
			MSG_DUMP("  xte                       : %f m\n", (float)(msg->data.crosstrack_error.xte)*0.01f);
			break;
		case PGN_DRIFT_RAPID_UPDATE_ID:
			nmea2000_data.drift_angle = (float)(msg->data.drift_rapid_update.set)*0.0001f;
			nmea2000_data.drift_speed = (float)(msg->data.drift_rapid_update.drift)*0.01f;
			break;
		case PGN_WIND_DATA_ID:
			nmea2000_data.AWS = (float)(msg->data.wind_data.speed)*0.01f;
			nmea2000_data.AWA = (float)(msg->data.wind_data.direction)*0.0001f;
		...
		case PGN_ENV_PARAM_ID:
			if (msg->data.env_param.temp_source == 0) {
				nmea2000_data.water_temp = KTOC((float)(msg->data.env_param.temperature)*0.01f);
			} else if (msg->data.env_param.temp_source == 1) {
				nmea2000_data.outside_air_temp = KTOC((float)(msg->data.env_param.temperature)*0.01f);
			} else if (msg->data.env_param.temp_source == 2) {
				nmea2000_data.inside_air_temp = KTOC((float)(msg->data.env_param.temperature)*0.01f);
			}
			if (msg->data.env_param.humidity_source == 0) {
				nmea2000_data.inside_air_humidity = (float)(msg->data.env_param.humidity)*0.004f;
			} else if (msg->data.env_param.humidity_source == 1) {
				nmea2000_data.outside_air_humidity = (float)(msg->data.env_param.humidity)*0.004f;
			}
			if (msg->data.env_param.atmospheric_pressure != 0xFFFF)
				nmea2000_data.atmospheric_pressure = (float)(msg->data.env_param.atmospheric_pressure)*100.0f;
			break;
		case PGN_TEMPERATURE_ID:
			if (msg->data.temperature.source == 1) {
				nmea2000_data.outside_air_temp = KTOC((float)(msg->data.temperature.temperature)*0.01f);
			}
			break;
		case PGN_TEMPERATURE_EXTENDED_ID:
			if ( msg->data.temperature_extended.source == 1) {
				nmea2000_data.outside_air_temp = KTOC((float)(msg->data.temperature_extended.temperature)*0.001f);
			}
			break;
	}
}
...
static void *data_viewer(void *arg)
{
	...
	memset(&nmea2000_stats_old, 0, sizeof(struct nmea2000_stats_s));
	memset(&ydwg_stats_old, 0, sizeof(struct ydwg_stats_s));
